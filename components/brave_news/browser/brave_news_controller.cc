...
#define IN_ENGINE_FF(Method)                                         \
  ...
  BraveNewsController::BraveNewsController(
      PrefService * prefs, favicon::FaviconService * favicon_service,
  ...
  void BraveNewsController::GetFollowingFeed(
      GetFollowingFeedCallback callback) {
  ...
  void BraveNewsController::GetPublisherFeed(
      const std::string& publisher_id, GetPublisherFeedCallback callback) {
  ...
  void BraveNewsController::SetChannelSubscribed(
      const std::string& locale, const std::string& channel_id, bool subscribed,
  ...
  void BraveNewsController::OnVerifiedDirectFeedUrl(
      const GURL& feed_url, SubscribeToNewDirectFeedCallback callback,
      const bool is_valid, const std::string& feed_title) {
  ...
  void BraveNewsController::GetImageData(const GURL& padded_image_url,
    ...
    const bool is_padded =
        (file_name.compare(file_name.length() - ending.length(),
  ...
  void BraveNewsController::IsFeedUpdateAvailable(
    ...
    IN_ENGINE(IsFeedV1UpdateAvailable, std::move(callback),
              displayed_feed_hash);
  }
  ...
  void BraveNewsController::GetDisplayAd(GetDisplayAdCallback callback) {
    ...
    auto on_ad_received = base::BindOnce(
        ...
        std::move(callback));
    ads_service_->MaybeServeInlineContentAd("900x750",
                                            std::move(on_ad_received));
  }
  ...
  void BraveNewsController::OnNewCardsViewed(uint16_t card_views) {
    news_metrics_.RecordTotalActionCount(p3a::ActionType::kCardView,
                                         card_views);
  }
  ...
  void BraveNewsController::OnSidebarFilterUsage() {
    news_metrics_.RecordTotalActionCount(p3a::ActionType::kSidebarFilterUsage,
                                         1);
  }
  ...
  void BraveNewsController::CheckForFeedsUpdate() {
    ...
    IN_ENGINE_FF(CheckForFeedsUpdate);
  }
  ...
  void BraveNewsController::NotifyChannelsChanged(
      mojom::ChannelsEventPtr event) {
