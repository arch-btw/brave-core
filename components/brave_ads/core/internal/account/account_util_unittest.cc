...
TEST_F(BraveAdsAccountUtilTest,
  ...
  for (int i = 0; i < static_cast<int>(ConfirmationType::kMaxValue); ++i) {
    EXPECT_TRUE(
        CanDeposit(AdType::kInlineContentAd, static_cast<ConfirmationType>(i)));
  }
}
...
TEST_F(BraveAdsAccountUtilTest,
  ...
  for (int i = 0; i < static_cast<int>(ConfirmationType::kMaxValue); ++i) {
    EXPECT_FALSE(
        CanDeposit(AdType::kNotificationAd, static_cast<ConfirmationType>(i)));
  }
}
...
TEST_F(
  ...
  for (int i = 0; i < static_cast<int>(ConfirmationType::kMaxValue); ++i) {
    EXPECT_TRUE(
        CanDeposit(AdType::kNewTabPageAd, static_cast<ConfirmationType>(i)));
  }
}
...
TEST_F(BraveAdsAccountUtilTest,
  ...
  for (int i = 0; i < static_cast<int>(ConfirmationType::kMaxValue); ++i) {
    EXPECT_FALSE(
        CanDeposit(AdType::kSearchResultAd, static_cast<ConfirmationType>(i)));
  }
}
